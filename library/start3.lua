    ...
    if (MINHUD_show_warp == true and warpdrive ~= nil) then
        ...
        if system.getTime() > self.last_time_updated + MINHUD_warp_refresh_rate then
            ...
            if err then
                ...
                self.html = self.html..[[<div class="warp_mod"><div>]]..obj.buttonMsg..[[</div><div style="color:red">]]..obj.errorMsg..[[</div><div>Dest : ]]..obj.destination..[[</div><div>Dist : ]]..getDistanceDisplayString(obj.distance)..[[</div><div>Cells : ]]..obj.cellCount..[[</div></div>]]
            end
        end    
    end
    return self.html
